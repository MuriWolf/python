...
precoGasolina = float(input("\nDigite o preço da gasolina: "))

for i in range(5):
    if i == 0:
        print("\nDigite o nome de 5 carros e a quantidade de km por litros deles.\n")
    print(f"\nVeiculo {i + 1}")
    nome = input("Nome: ")
    nomes.append(nome)

    kmLitro = float(input("Km por litro: "))
    kmLitros.append(kmLitro)

for i in range(len(nomes)):
    if i == 0:
        print("\nRelatório final")
    MilKm = 1000 / kmLitros[i]

    print(f"\n{i+1} - {nomes[i]} - {kmLitros[i]} - {int(MilKm)} litros - R$ {int(MilKm * precoGasolina)}")